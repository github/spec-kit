...
- **[Specstride](https://github.com/mairp/specstride)** — An autonomous orchestrator that runs a coding agent (Claude Code, Codex, or a local model) through a feature's `tasks.md` one phase at a time, with `spec.md`, `plan.md` and contracts as read-only context. Each phase advances only when a separate LLM critic approves its evidence against the acceptance criteria and the code. Stuck phases get a diagnostician and a narrowed retry. An opt-in loop learns per-phase settings from run telemetry and reverts any change that makes things worse.
