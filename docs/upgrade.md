...
| **CLI Tool — pin a version** | `specify self upgrade --tag vX.Y.Z[suffix]` | Upgrade to a specific release tag instead of the latest stable. Replace `vX.Y.Z[suffix]` with the release tag you want. |
...
Pinned tags accept the same release-tag form that Spec Kit publishes, including prerelease and build suffixes such as `v1.0.0-rc1`, `v0.8.0.dev0`, or `v0.8.0+build.42`.
