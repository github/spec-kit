...
Validates the fix by re-running the reproduction and any added tests, then records the verification result. Like `assess`, it is **read-only** with respect to source code. Verdicts are never over-claimed — a reproduction that was not actually performed is reported as `partial` or `not-run`, not `verified`.
