...
Creates or updates the feature **specification** from a natural-language description. Focus on the **what** and **why** — the user-facing behavior and goals — not the tech stack, which belongs in `/speckit.plan`. After plan, tasks, or implementation exist, use `/speckit.revise` to change FRs, SCs, or ACs.
...
Run this when FRs, SCs, or ACs change **after** plan, tasks, or implementation exist. Then `/speckit.analyze` (if plan and tasks exist), `/speckit.implement` (skips cancelled tasks), then `/speckit.converge`. Do not run `/speckit.converge` first — converge assumes the spec is stable.
