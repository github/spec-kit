...
Creates a **new** feature **specification** from a natural-language description (a new directory under `specs/`). Focus on the **what** and **why** — the user-facing behavior and goals — not the tech stack, which belongs in `/speckit.plan`. To add, remove, or reword requirements on the **current** spec, run `/speckit.revise` instead.

This workflow may also maintain `checklists/requirements.md`, the built-in spec-quality checklist that `/speckit.specify` creates and `/speckit.clarify` / `/speckit.revise` re-evaluate. That lifecycle is separate from custom checklists generated by `/speckit.checklist`.
...
Clarifying before planning keeps you from designing on top of ambiguity. If `/speckit.analyze` later surfaces requirement gaps, come back and run `/speckit.clarify` (pre-plan questions), `/speckit.revise` (a known add/remove/reword on the current spec), or `/speckit.specify` (a genuinely new feature).
