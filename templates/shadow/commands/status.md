...
scripts:
  sh: {SHADOW_PATH}/scripts/bash/workflow-status.sh --json
  ps: {SHADOW_PATH}/scripts/powershell/workflow-status.ps1 -Json
---
...
7. **Provide contextual guidance**:
   - If no feature branch: Suggest creating one with `/specify`
   - If spec has clarifications: Suggest `/clarify`
   - If token budget > 80%: Warn and suggest `/prune`
   - If validation not run: Suggest `/validate`
...
|-------|------|-----------|-----------|
| 0 | Setup | No constitution.md | `/constitution` |
| 1 | Ready for Feature | Constitution exists, no feature branch | `/specify` |
| 2 | Specification Complete | spec.md exists, no plan.md | `/plan` |
| 3 | Planning Complete | plan.md exists, no tasks.md | `/tasks` |
| 4 | Ready for Implementation | tasks.md exists, 0% complete | `/implement` |
| 5 | Implementation in Progress | tasks.md exists, 1-99% complete | `/implement` |
| 6 | Implementation Complete | tasks.md exists, 100% complete | `/document` |
...
• Run /budget for detailed token breakdown
...
1. **No git repository**:
   ...
   This toolkit uses git branches to organize features.
...
2. **No constitution or features**:
   ...
   2. /specify "your feature" - Create your first feature
