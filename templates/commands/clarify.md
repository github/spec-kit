...
- Typical levels here: scanning the spec for ambiguity and composing the question set → `3`; encoding an accepted answer back into the spec → `2`.
...
- Keep in the manager only: reading configuration, classifying steps, asking the user the clarification questions, merging worker output, and the final report. Never delegate the user-facing questioning itself.
