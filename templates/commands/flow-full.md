...
- **User interaction is never suppressed**: if any phase needs a user decision, clarification, or confirmation (per its own instructions), ASK and wait. This applies especially to `clarify`, whose questions MUST be answered by the user — never auto-answer them. Automation chains phases; it does not silence questions.
...
Execute `__SPECKIT_COMMAND_TASKS__`. Every task must carry its `[C:complexity->model]` label per the models.json mapping.
