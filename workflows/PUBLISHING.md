...
- **Prefer explicit commands over interpolation** in `run` blocks: expressions are spliced in as raw text with no automatic quoting, so `{{ inputs.something }}` substitutions should be quoted (`'{{ inputs.something }}'`) and constrained via `enum` so a malicious input can't inject shell syntax.
- **Treat prior-step output as untrusted too** — `{{ steps.*.output.* }}` from a `prompt` step is AI-generated text that upstream content can influence. Don't interpolate agent output straight into a `run` field; quote it, validate it, or route it through a `gate` step first.
