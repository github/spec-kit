...
""")
    ...
    def test_branch_pattern_validation_workflow(self, config_service: ConfigService):
        ...
        for pattern in valid_patterns:
            ...
            assert isinstance(is_valid, bool), f"Validation failed for pattern: {pattern}"
            if not is_valid:
    ...
    def test_branch_pattern_configuration_workflow(self, config_service: ConfigService, tmp_path: Path):
        ...
        test_expansions = [
            (loaded_config.branch_naming.default_pattern, {"feature-name": "test"}, "feature/test"),
            ("epic/{epic-name}/{feature-name}", {"epic-name": "user-auth", "feature-name": "login"}, "epic/user-auth/login"),
            ("team/{team}/{task-name}", {"team": "backend", "task-name": "api-refactor"}, "team/backend/api-refactor")
        ]
    ...
    def test_branch_naming_integration_with_templates(self, config_service: ConfigService, tmp_path: Path):
        ...
        branch_result = config_service.expand_branch_name(
            "feature/{feature-name}",
            {"feature-name": "template-integration"}
        )
        ...
        branch_result = config_service.expand_branch_name(
            "team/{team-name}/{task}",
            {"team-name": "backend", "task": "refactor"}
        )
    ...
    def test_complex_branch_naming_scenarios(self, config_service: ConfigService):
        ...
        for scenario in all_scenarios:
            ...
            is_valid, error = config_service.validate_branch_pattern(scenario["pattern"])
            assert is_valid, f"Pattern should be valid: {scenario['pattern']}, Error: {error}"
            
            # Test expansion
            result = config_service.expand_branch_name(scenario["pattern"], scenario["context"])
