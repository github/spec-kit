...
"""
    ...
    def test_comprehensive_template_rendering(self, template_service: TemplateService, comprehensive_template_package: Path, rich_template_context: TemplateContext):
        """Test comprehensive Jinja2 template rendering with complex features"""
        # Load template package
        success = template_service.load_template_package("claude", comprehensive_template_package)
        assert success, "Failed to load comprehensive template package"
        
        # Test individual template rendering
        readme_content = template_service.render_template("README.md.j2", rich_template_context)
        ...
        package_content = template_service.render_template("package.json.j2", rich_template_context)
        ...
        config_content = template_service.render_template("config.toml.j2", rich_template_context)
        ...
        spec_content = template_service.render_template("spec.md.j2", rich_template_context)
        ...
        script_content = template_service.render_template("scripts/setup.sh.j2", rich_template_context)
    ...
    def test_template_rendering_with_missing_variables(self, template_service: TemplateService, comprehensive_template_package: Path):
        ...
        readme_content = template_service.render_template("README.md.j2", minimal_context)
        ...
        package_content = template_service.render_template("package.json.j2", minimal_context)
    ...
    def test_template_rendering_edge_cases(self, template_service: TemplateService, tmp_path: Path):
...
""")
    ...
    def test_batch_template_rendering(self, template_service: TemplateService, comprehensive_template_package: Path, rich_template_context: TemplateContext, tmp_path: Path):
        ...
        template_files = template_service.render_project_templates(rich_template_context, output_dir)
        ...
        readme_file = next((tf for tf in template_files if 'README.md' in tf.output_path), None)
        assert readme_file is not None
        assert "# integration-test-project" in readme_file.content
        
        package_file = next((tf for tf in template_files if 'package.json' in tf.output_path), None) 
        assert package_file is not None
        assert '"name": "integration-test-project"' in package_file.content

    def test_template_syntax_validation_integration(self, template_service: TemplateService, tmp_path: Path):
        ...
        for filename in invalid_templates.keys():
            template_path = template_dir / filename
    ...
    def test_template_variable_extraction_integration(self, template_service: TemplateService, comprehensive_template_package: Path):
        ...
        expected_vars = {
            "project_name", "ai_assistant", "branch_type", "feature_name",
