...
class TestCopilotSkillsMode:
    ...
    def test_post_process_skill_content_does_not_inject_mode(self):
        """post_process_skill_content() should not inject unsupported mode field."""
        copilot = self._make_copilot()
        content = (
            "---\n"
            'name: "speckit-plan"\n'
    ...
    def test_post_process_skill_content_injects_hook_note(self):
        """post_process_skill_content() should only inject shared hook guidance."""
        copilot = self._make_copilot()
        content = (
            "---\n"
    ...
    def test_skills_omit_mode_frontmatter(self, tmp_path):
        """Generated SKILL.md files should omit unsupported mode field."""
        copilot = self._make_copilot()
        created, _ = self._setup_skills(copilot, tmp_path)
        skill_files = [f for f in created if f.name == "SKILL.md"]
        assert len(skill_files) > 0
        for f in skill_files:
            ...
            parts = content.split("---", 2)
            fm = yaml.safe_load(parts[1])
            assert "mode" not in fm, f"{f} frontmatter contains unsupported 'mode'"
