...
class TestOpencodeSkillsMode:
    ...
    def test_dispatch_command_no_project_root_skills_mode(self):
        from unittest import mock

        integration = get_integration(self.KEY)
        integration._skills_mode = True  # fallback used when no project_root

        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.MagicMock(returncode=0)
            integration.dispatch_command("speckit.plan", "test args", stream=False)

        called_args = mock_run.call_args[0][0]
        assert "--command" in called_args
        assert called_args[called_args.index("--command") + 1] == "speckit-plan"
        assert integration._skills_mode is True  # dispatch must not mutate _skills_mode

    def test_dispatch_command_no_project_root_default_mode(self):
        from unittest import mock

        integration = get_integration(self.KEY)
        integration._skills_mode = False  # fallback used when no project_root
        ...
        assert "--command" in called_args
        assert called_args[called_args.index("--command") + 1] == "speckit.plan"
        assert integration._skills_mode is False  # dispatch must not mutate _skills_mode
