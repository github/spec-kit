...
async function runPublish({ item, eventSha, currentSha = eventSha, state = 'open', labels = [], action = 'labeled', labelName = 'community-review' }) {
  const { github, calls } = fakeGitHub({ sha: currentSha, state, labels });
  ...
  const env = { GH_AW_AGENT_OUTPUT: outputPath, GH_AW_EXPECTED_HEAD_SHA: eventSha, GH_AW_PR_NUMBER: '7' };
...
test('valid output publishes one comment and one current outcome label', async () => {
  ...
  const calls = await runPublish({ eventSha: sha, item: { type: 'community_assess_publish', expected_head_sha: sha, outcome: 'fits-project', body: 'evidence' } });
  assert.equal(calls.comments.length, 1);
  assert.deepEqual(calls.added, ['community-assessment-fits']);
  assert.ok(calls.gets >= 4);
});

test('delayed old publisher preserves a newer head outcome', async () => {
  const currentSha = 'b'.repeat(40);
  const eventSha = 'a'.repeat(40);
  const calls = await runPublish({ eventSha, currentSha, labels: ['community-assessment-fits'], item: { type: 'community_assess_publish', expected_head_sha: eventSha, outcome: 'fits-project', body: 'stale' } });
...
test('closed PR fails the fresh check and clears current outcomes', async () => {
  ...
  const calls = await runPublish({ eventSha: sha, state: 'closed', labels: ['community-assessment-invalid'], item: { type: 'community_assess_publish', expected_head_sha: sha, outcome: 'invalid', body: 'closed' } });
  assert.equal(calls.comments.length, 0);
...
test('invalid output is ignored without a GitHub mutation', async () => {
  ...
  const calls = await runPublish({ eventSha: sha, labels: ['community-assessment-fits'], item: { type: 'community_assess_publish', expected_head_sha: sha, outcome: 'not-allowed', body: 'invalid' } });
  assert.equal(calls.comments.length, 0);
  assert.deepEqual(calls.added, []);
  assert.deepEqual(calls.removed, []);
});
...
test('retrigger removes the previous outcome before applying the new one', async () => {
  ...
  const calls = await runPublish({ eventSha: sha, labels: ['community-assessment-out-of-scope'], item: { type: 'community_assess_publish', expected_head_sha: sha, outcome: 'needs-clarification', body: 'new' } });
...
test('workflow publication remains inert until a trusted fork context is approved', () => {
  assert.equal(existsSync(join(root, '.github', 'workflows', 'community-assess.lock.yml')), false);
  assert.equal(existsSync(join(root, '.github', 'workflows', 'community-assess-cleanup.yml')), false);
  const frontmatter = workflow.split('---').slice(0, 2).join('---');
  assert.match(frontmatter, /intentionally inert/);
  assert.doesNotMatch(frontmatter, /pull_request_target/);
});
