...
find_feature_dir_by_prefix() {
    ...
    # Extract prefix from branch (e.g., "004" from "004-whatever", "URA-004" from "URA-004-whatever",
    # or "20260319-143022" from timestamp branches)
    local prefix=""
    if [[ "$dir_name" =~ ^([0-9]{8}-[0-9]{6})- ]]; then
        prefix="${BASH_REMATCH[1]}"
    else
        # Strip optional acronym prefix (e.g., "URA-" from "URA-004-whatever")
        local stripped_name
