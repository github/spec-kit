...
A developer runs `/speckit.clarify` and the system identifies ambiguities across 5 categories (Functional Scope, Data Model, UX Flow, Non-Functional, Edge Cases). Research for each category runs concurrently to identify candidate clarification questions faster.
...
A developer runs `/speckit.taskstoissues` with 15 tasks. Instead of creating issues one at a time, up to 10 concurrent subagents create GitHub issues in parallel, respecting API rate limits.
...
1. **Given** a tasks.md with 15 tasks, **When** user runs `/speckit.taskstoissues`, **Then** up to 10 issues create concurrently
...
- What happens when the 10-subagent Claude Code limit is reached? → Queue overflow tasks, start queued task when slot frees (greedy refill)
- What happens when subagents produce conflicting results? → Parent agent reconciles conflicts, flags unresolvable conflicts to user
...
- **FR-003**: System MUST limit concurrent subagents to Claude Code's hard cap of 10
...
- **FR-006**: System MUST provide consistent progress reporting format across all parallel commands
...
- **FR-012**: `/speckit.taskstoissues` MUST create GitHub issues concurrently (up to 10 at a time)
...
- **Subagent Task**: A discrete unit of work dispatched to a Task tool subagent (id, prompt, context_scope, timeout)
...
- **SC-006**: All parallel commands produce identical output to sequential mode (correctness preserved)
