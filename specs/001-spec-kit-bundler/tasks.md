...
Generated by `converge` — gaps between the spec/plan/constitution and the
current code, appended as new tasks for `implement` to complete. Ordered
...
- [x] T052 Make `specify bundle update` actually refresh each declared component
  ...
  `is_installed` is already true, so no refresh occurs).

## Phase 9: Convergence

Generated by `converge` — a residual gap found after Phase 8, appended as a new
task for `implement` to complete. No existing task was modified and no code was
changed by convergence itself.

- [x] T053 Surface the bundle verification/trust indicator on the discovery
  surface: include `verified` (and the source catalog/trust framing) in
  `specify bundle search` output (both human-readable text and `--json`) and in
  `specify bundle info`'s human-readable text, so users can judge a bundle's
  trust level before installing, per FR-010 / FR-027 (partial — `entry.verified`
  is currently exposed only in `bundle info --json`; `bundle search` omits it
  entirely and `bundle info` text does not display it).
