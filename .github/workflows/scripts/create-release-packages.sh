...
build_variant() {
  ...
  [[ -d templates ]] && { mkdir -p "$SPEC_DIR/templates"; find templates -type f -not -path "templates/commands/*" -not -name "vscode-settings.json" | while read f; do mkdir -p "$SPEC_DIR/$(dirname "$f")"; cp "$f" "$SPEC_DIR/$f"; done; echo "Copied templates -> .specify/templates"; }
