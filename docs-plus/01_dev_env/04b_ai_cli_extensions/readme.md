...
#### **Example 1: Code Completion (Proprietary)**
...
**✅ Hybrid approach: MCP for most, proprietary for edge cases**
...
**Extension model: Custom tools via configuration**
...
### Appendix A: MCP Server Implementation Checklist

**Before writing code:**
- [ ] Search existing MCP servers (might already exist)
- [ ] Review MCP specification
- [ ] Choose transport (stdio for local, HTTP for remote)
- [ ] Select SDK (Python or TypeScript recommended)
- [ ] Define security requirements

**During implementation:**
- [ ] Implement resource listing
- [ ] Implement resource reading (with caching if appropriate)
...
- [ ] Choose appropriate license (MIT/Apache 2.0 recommended)

### Appendix B: Extension Debugging Guide

**Common issues and solutions:**

**Issue: Extension not discovered by agent**
```bash
...
```bash
# Test server directly with MCP client
mcp-client test custom_server.py --tool create_user --args '{"email":"test@example.com"}'

# Check input schema validation
mcp-client validate custom_server.py
```

**Issue: Performance problems**
```bash
# Profile server
mcp-profile custom_server.py --duration 60s

# Check resource usage
mcp-monitor custom_server.py
```

### Appendix C: Migration Guide (Native → MCP)

**Step 1: Audit existing extensions**
```bash
# List all platform-specific extensions
ls ~/.config/*/extensions/

# Document functionality
for ext in extensions/*; do
  echo "Extension: $ext"
  echo "Tools: $(count_tools $ext)"
  echo "Usage: $(check_usage_stats $ext)"
done
```
...
- Single-platform specific
```

**Step 3: Implement MCP version**
```python
# Create MCP server with same functionality
# Test thoroughly
# Deploy alongside native version
```
...
# Week 3: 50% of users
# Week 4: 100% of users
# Week 5: Deprecate native version
```
