...
1. Load `.specify/ai-team/tasks/<task-id>/task-context.yml` and bug artifacts under `.specify/bugs/<bug_slug>/` when present.
...
2. Write `.specify/ai-team/evidence/<bug_slug>/evidence-board.md` (use `bug_slug` when no feature directory exists).
