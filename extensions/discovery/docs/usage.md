...
Use the `discovery` extension before formal planning when a product idea, technical direction, technology choice, legacy integration, or vertical technology decision risk has important unknowns.

The extension provides source-backed foundation commands:
...
It also provides vertical technology decision commands for common API, performance, migration, UX, and compatibility situations:

- `/speckit.discovery.api`: API, SDK, webhook, service, SaaS, or partner integration technology decision.
- `/speckit.discovery.performance`: latency, throughput, scalability, capacity, or cost technology decision.
- `/speckit.discovery.migration`: data model, storage, schema, backfill, or migration technology decision.
- `/speckit.discovery.ux`: complex UX interaction, workflow, accessibility, or frontend/backend handoff technology decision.
- `/speckit.discovery.compatibility`: browser, OS, device, runtime, version, or deployment compatibility technology decision.
...
- vertical technology decision commands: evaluate API, performance, migration, UX, and compatibility decisions with scenario-specific risks.
...
## Vertical Technology Decision Command Selection

Use a vertical technology decision command when the user already knows the shape of the technical decision:

| Situation | Command | Typical output |
|---|---|---|
| External API, SDK, webhook, SaaS, partner, or service dependency | `/speckit.discovery.api` | `api-integration-discovery.md` |
| Latency, throughput, load, scalability, resource, or cost assumption | `/speckit.discovery.performance` | `performance-discovery.md` |
| Schema, model, storage, backfill, import/export, or data migration risk | `/speckit.discovery.migration` | `data-migration-discovery.md` |
| Complex interaction, stateful workflow, accessibility, responsive behavior, or handoff risk | `/speckit.discovery.ux` | `ux-discovery.md` |
| Browser, OS, device, runtime, dependency, deployment, or version support risk | `/speckit.discovery.compatibility` | `compatibility-discovery.md` |

Each vertical technology decision command is independent and should set its own command-specific `Planning Decision`. Each command sets `Evaluation Mode` to `comparison` when evaluating multiple candidates, or `single-approach-readiness` when evaluating one approach against acceptance criteria. Use follow-up foundation commands only when the vertical technology decision assessment recommends them.
...
```text
/speckit.discovery.api Target: Stripe webhooks. Workflow: mark invoices paid after payment_succeeded. Risks: retries, duplicate events, signature verification, local replay tests.
```

Performance:

```text
/speckit.discovery.performance Concern: search must respond under 200ms p95. Target flow: note search while typing. Load: 50k local notes and 10 concurrent queries.
```

Data migration:

```text
/speckit.discovery.migration Change: split customer address into normalized table. Current: address JSON on customer records. Constraints: zero downtime, rollback within one release.
```

UX:

```text
/speckit.discovery.ux Workflow: bulk edit selected records. Concern: partial failures, undo, keyboard access, and responsive toolbar behavior. Success: operators can recover from failed rows.
```

Compatibility:

```text
/speckit.discovery.compatibility Target: Safari and Chromium browsers. Feature: offline note sync. Environments: latest two major versions on desktop and mobile.
```
...
1. Run a vertical technology decision command when the decision shape is obvious, such as `/speckit.discovery.api` or `/speckit.discovery.performance`.
2. Run `/speckit.discovery.feasibility` when the main question is whether the idea supports a go/no-go continuation decision.
3. Run `/speckit.discovery.techselect` when a general technology choice needs an explicit comparison, or when a vertical command finds multiple tools or architectures that need broader comparison.
