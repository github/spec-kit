...
- Any changes to `__init__.py` for the Diagramly CLI require a version rev in `pyproject.toml` and addition of entries to `CHANGELOG.md`.
...
Diagramly supports multiple AI agents by generating agent-specific command files and directory structures when initializing projects. Each agent has its own conventions for:
...
Add the new agent to the `AGENT_CONFIG` dictionary in `src/diagramly_cli/__init__.py`. This is the **single source of truth** for all agent metadata:
...
2. **CLI test**: Test `diagramly init --ai <agent>` command
