...
The Specify CLI carries **no agent-context state**: it never creates, updates, removes, or resolves context/instruction files (`CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md`, …). New integrations add nothing for context handling.

Managing the "Spec Kit" section in the context file is fully owned by the **`agent-context` extension** (`extensions/agent-context/`). The extension ships with Spec Kit but is a **full opt-in** — `specify init` does not install or activate it. A user adds/enables it through the standard extension verbs, after which the extension's own bundled scripts maintain the context section. When the extension is absent or disabled, nothing in Spec Kit touches the context file.
...
- The Specify CLI does **not** write this config. When `context_file` is empty, the extension's bundled scripts self-seed it by reading the active integration key from `.specify/init-options.json` (written by `specify init`) and mapping it to a default context file via the extension's own `agent-context-defaults.json` (`extensions/agent-context/scripts/bash/update-agent-context.sh` and `.ps1`). The Specify CLI registry is never consulted — all agent→context-file knowledge lives inside the extension.
...
2. **Forgetting that context configuration is extension-owned**: The bundled `agent-context` extension reads from `.specify/extensions/agent-context/agent-context-config.yml`. The CLI does not write this file — the extension self-seeds it from `agent-context-defaults.json`. New integrations do not need to set `context_file` or any context-related attributes.
