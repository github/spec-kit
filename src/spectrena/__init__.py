...
# ///
"""
Spectrena CLI - Setup tool for Spectrena projects

Usage:
    uvx spectrena.py init <project-name>
    uvx spectrena.py init .
    uvx spectrena.py init --here

Or install globally:
    uv tool install --from spectrena.py spectrena
    spectrena init <project-name>
    spectrena init .
    spectrena init --here
"""
...
TAGLINE = "Spec-Driven Development Enhanced"
...
class StepTracker:
    ...
    def __init__(self, title: str):
        ...
        self._refresh_cb: Callable[[], None] | None = (
            None  # callable to trigger UI refresh
        )
...
app = typer.Typer(
    name="spectrena",
    help="Setup tool for Spectrena spec-driven development projects",
...
def callback(ctx: typer.Context):
    ...
    if (
        ...
        show_banner()
        console.print(
            Align.center("[dim]Run 'spectrena --help' for usage information[/dim]")
        )
        console.print()
...
def init_git_repo(project_path: Path, quiet: bool = False) -> tuple[bool, str | None]:
    ...
    try:
        ...
        subprocess.run(
            ["git", "commit", "-m", "Initial commit from Spectrena template"],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        if commands_src.exists():
            ...
            for cmd_file in commands_src.glob("*.md"):
                ...
                if ai_assistant in ["claude", "copilot", "cursor-agent", "opencode", "windsurf", "codex", "kilocode", "auggie", "roo", "codebuddy", "amp", "shai", "q", "bob"]:
                    ...
                    output_file = commands_dest / f"spectrena.{cmd_file.stem}.toml"
...
def ensure_executable_scripts(
    ...
    """Ensure POSIX .sh scripts under .spectrena/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".spectrena" / "scripts"
...
def init(
    ...
    Examples:
        spectrena init my-project
        spectrena init my-project --ai claude
        spectrena init my-project --ai copilot --no-git
        spectrena init --ignore-agent-tools my-project
        spectrena init . --ai claude         # Initialize in current directory
        spectrena init .                     # Initialize in current directory (interactive AI selection)
        spectrena init --here --ai claude    # Alternative syntax for current directory
        spectrena init --here --ai codex
    ...
    if here and project_name:
        console.print(
            "[red]Error:[/red] Cannot spectrena both project name and --here flag"
        )
        raise typer.Exit(1)

    if not here and not project_name:
        console.print(
            "[red]Error:[/red] Must spectrena either a project name, use '.' for current directory, or use --here flag"
        )
        raise typer.Exit(1)
    ...
    if here:
        ...
        assert project_name is not None  # Validated earlier
    ...
    setup_lines = [
        "[cyan]Spectrena Project Setup[/cyan]",
        "",
    ...
    tracker = StepTracker("Initialize Spectrena Project")

    sys._spectrena_tracker_active = True  # pyright: ignore[reportAttributeAccessIssue]
    ...
    enhancement_lines = [
        ...
        "",
        "○ [cyan]/spectrena.clarify[/] [bright_black](optional)[/bright_black] - Ask structured questions to de-risk ambiguous areas before planning (run before [cyan]/spectrena.plan[/] if used)",
        "○ [cyan]/spectrena.analyze[/] [bright_black](optional)[/bright_black] - Cross-artifact consistency & alignment report (after [cyan]/spectrena.tasks[/], before [cyan]/spectrena.implement[/])",
        "○ [cyan]/spectrena.checklist[/] [bright_black](optional)[/bright_black] - Generate quality checklists to validate requirements completeness, clarity, and consistency (after [cyan]/spectrena.plan[/])",
    ]
...
def check():
    ...
    _ = check_tool("code", tracker=tracker)
