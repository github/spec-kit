...
AGENT_COMMAND_CONFIG = {
    ...
    "goose": {"dir": ".goose/recipes", "ext": "yaml", "args": "{{args}}"},
...
def _human_title(identifier: str) -> str:
    """Convert a command identifier into a human-readable title."""
    text = identifier
...
def _render_yaml_recipe(title: str, description: str, body: str, source_id: str) -> str:
    """Render a Goose recipe YAML file."""
    header = {
        "version": "1.0.0",
        "title": title,
    ...
    return (
        "\n".join([header_yaml, "prompt: |", indented, "", f"# Source: {source_id}"])
        + "\n"
    )
...
def _process_command_template(
    ...
    script_cmd = (meta.get("scripts") or {}).get(
    ...
    if ext == "toml":
        ...
        toml_body = _strip_frontmatter_block(body)
        ...
        yaml_body = _strip_frontmatter_block(body)
        title = _human_title(f"speckit.{name}")
        description = meta.get("description", "")
        out_file.write_text(
            _render_yaml_recipe(
                title,
                description,
                yaml_body,
                f"templates/commands/{template_path.name}",
            ),
            encoding="utf-8",
        )
    else:
...
_RE_CONSTITUTION_VERSION = re.compile(r"\*\*Version\*\*:\s*(\d+)\.(\d+)\.(\d+)")
_RE_CONSTITUTION_PLACEHOLDER = re.compile(r"\[CONSTITUTION_VERSION\]")
        ...
        if commands_dir.is_dir():
            for template_file in sorted(commands_dir.glob("*.md")):
                _process_command_template(
                    template_file, agent, script_type, output_dir, config
                )
            ...
            if vscode_src.is_file():
                ...
                if vscode_dest.exists():
                    handle_vscode_settings(
                        vscode_src, vscode_dest, Path("settings.json")
                    )
                else:
...
def get_git_version() -> Optional[Tuple[int, int, int]]:
    ...
    try:
        ...
        if version_str.startswith("git version "):
            version_part = version_str[12:].split()[
                0
            ]  # Get first part after "git version "
                            ...
                            if dest_path.exists():
                                ...
                                for sub_item in item.rglob("*"):
                                    if sub_item.is_file():
                                        ...
                                        elif _is_constitution_file(
                                            dest_file
                                        ) and _is_customized_constitution(dest_file):
                                            if not _confirm_constitution_overwrite(
                                                dest_file
                                            ):
                                                continue
                                        else:
...
def init(
    ...
    ai_assistant: str = typer.Option(
        None,
        "--ai",
        help="AI assistant to use: claude, gemini, goose, copilot, cursor-agent, qwen, opencode, codex, windsurf, kilocode, auggie, codebuddy, amp, shai, q, bob, or qoder ",
    ),
    ai_skills: bool = typer.Option(
        False,
        "--ai-skills",
        help="Generate agent skills from command templates",
    ),
    ai_commands_dir: str = typer.Option(
        None,
        "--ai-commands-dir",
        help="Custom commands directory to use with --ai generic",
    ),
    script_type: str = typer.Option(
        None, "--script", help="Script type to use: sh or ps"
    ),
    git_mode: str = typer.Option(
        None,
        "--git-mode",
        help="Git workflow mode: branch (traditional) or worktree (parallel directories)",
    ),
    worktree_strategy: str = typer.Option(
        None,
        "--worktree-strategy",
        help="Worktree location strategy: sibling, nested, or custom",
    ),
    worktree_path: str = typer.Option(
        None,
        "--worktree-path",
        help="Custom worktree base path (required if --worktree-strategy is 'custom')",
    ),
    ...
    if ai_skills and not ai_assistant:
        console.print("[red]Error:[/red] --ai-skills requires --ai")
        console.print(
            "[yellow]Usage:[/yellow] specify init <project-name> --ai claude --ai-skills"
        )
        raise typer.Exit(1)
    ...
    if selected_ai == "generic":
        if not ai_commands_dir:
            console.print("[red]Error:[/red] --ai generic requires --ai-commands-dir")
            raise typer.Exit(1)

        generic_dir = ai_commands_dir.rstrip("/")
        generic_path = Path(generic_dir)
        generic_parent = generic_path.parent.as_posix()
        AGENT_CONFIG["generic"]["folder"] = (
            f"{generic_parent}/" if generic_parent not in ("", ".") else ""
        )
        AGENT_CONFIG["generic"]["commands_subdir"] = generic_path.name or "commands"
        AGENT_COMMAND_CONFIG["generic"] = {
            "dir": generic_dir,
            "ext": "md",
            "args": "$ARGUMENTS",
        }
    ...
    if no_git and git_mode == "worktree":
        console.print(
            "[red]Error:[/red] Cannot use --git-mode worktree with --no-git (worktrees require git)"
        )
        raise typer.Exit(1)
    ...
    if git_mode:
        if git_mode not in GIT_MODE_CHOICES:
            console.print(
                f"[red]Error:[/red] Invalid git mode '{git_mode}'. Choose from: {', '.join(GIT_MODE_CHOICES.keys())}"
            )
            raise typer.Exit(1)
        ...
        if git_mode == "worktree":
            if not git_available:
                console.print(
                    "[red]Error:[/red] Cannot use --git-mode worktree: Git is not installed"
                )
                raise typer.Exit(1)
            if not worktree_supported:
                console.print(
                    f"[red]Error:[/red] Cannot use --git-mode worktree: {worktree_error_msg}"
                )
                raise typer.Exit(1)
        ...
        if sys.stdin.isatty() and git_available:
            if worktree_supported:
                selected_git_mode = select_with_arrows(
                    GIT_MODE_CHOICES, "Choose git workflow", "branch"
                )
            else:
                # Git available but worktree not supported - skip selection, default to branch
                console.print(
                    f"[yellow]Note:[/yellow] {worktree_error_msg}. Using branch mode."
                )
    ...
    if selected_git_mode == "worktree":
        if worktree_strategy:
            if worktree_strategy not in WORKTREE_STRATEGY_CHOICES:
                console.print(
                    f"[red]Error:[/red] Invalid worktree strategy '{worktree_strategy}'. Choose from: {', '.join(WORKTREE_STRATEGY_CHOICES.keys())}"
                )
                raise typer.Exit(1)
            ...
            if sys.stdin.isatty():
                selected_worktree_strategy = select_with_arrows(
                    WORKTREE_STRATEGY_CHOICES, "Choose worktree location", "sibling"
                )
        ...
        if selected_worktree_strategy == "custom":
            if worktree_path:
                ...
                if not Path(worktree_path).is_absolute():
                    console.print(
                        f"[red]Error:[/red] --worktree-path must be an absolute path (got: {worktree_path})"
                    )
                    raise typer.Exit(1)
                ...
                if not parent.exists():
                    console.print(
                        f"[red]Error:[/red] Parent directory does not exist: {parent}"
                    )
                    raise typer.Exit(1)
                ...
                if sys.stdin.isatty():
                    console.print()
                    selected_worktree_path = typer.prompt(
                        "Enter custom worktree base path (absolute path)"
                    )
                    if not Path(selected_worktree_path).is_absolute():
                        console.print(
                            f"[red]Error:[/red] Path must be absolute (got: {selected_worktree_path})"
                        )
                        raise typer.Exit(1)
                    ...
                    console.print(
                        "[red]Error:[/red] --worktree-path is required when --worktree-strategy is 'custom'"
                    )
                    raise typer.Exit(1)
        ...
        if selected_worktree_strategy == "sibling":
            ...
            preview_path = (
                Path(selected_worktree_path) / f"{repo_name}-<feature-branch>"
            )

        console.print()
        console.print(
            f"[cyan]Worktree preview:[/cyan] Features will be created at [green]{preview_path}[/green]"
        )
    ...
    tracker_steps = [
        ...
        ("ai-skills", "Generate agent skills"),
    ...
    tracker_steps.extend([("git", "Initialize git repository"), ("final", "Finalize")])
        ...
        try:
            ...
            if ai_skills:
                ...
                if skills_ok and not here:
                    agent_config = AGENT_CONFIG.get(selected_ai, {})
                    agent_folder = agent_config.get("folder", "") or ""
                    commands_subdir = agent_config.get("commands_subdir", "commands")
                    commands_dir = (
                ...
                tracker.skip("ai-skills", "not requested")
            ...
            if (
                selected_git_mode == "worktree"
                and selected_worktree_strategy == "nested"
            ):
                ...
                try:
                    ...
                    if (
                        worktree_entry in existing_content
                        or ".worktrees" in existing_content
                    ):
    ...
    if selected_git_mode == "worktree":
        ...
        location_desc = strategy_desc.get(
            selected_worktree_strategy, "in a separate directory"
        )

        # Use the correct script path based on selected script type
        configure_script = (
            "scripts/powershell/configure-worktree.ps1"
            if selected_script == "ps"
            else "scripts/bash/configure-worktree.sh"
        )
    ...
    if show_available:
        ...
        if not all_extensions:
            available_extensions = [
                ext for ext in available_extensions if ext["id"] not in installed_ids
            ]
        ...
        if available_extensions:
            ...
            for ext in available_extensions:
                verified_badge = (
                    " [green]✓ Verified[/green]" if ext.get("verified") else ""
                )
                installed_badge = (
                    " [dim](installed)[/dim]" if ext["id"] in installed_ids else ""
                )
                console.print(
                    f"  [bold]{ext['name']}[/bold] (v{ext.get('version', 'unknown')}){verified_badge}{installed_badge}"
                )
                console.print(f"     {ext.get('description', 'No description')}")
                console.print(f"     ID: {ext['id']}")
                if ext["id"] not in installed_ids:
                    console.print(
                        f"     Install: [cyan]specify extension add {ext['id']}[/cyan]"
                    )
                console.print()
        else:
            message = (
                "No additional available extensions found."
                if all_extensions
                else "No available extensions found."
            )
...
def extension_add(
    extension: Optional[str] = typer.Argument(
        None, help="Extension name or path (optional with --from)"
    ),
        ...
        with console.status(f"[cyan]Installing extension: {target}[/cyan]"):
            if dev:
                ...
                if extension is None:
                    console.print(
                        "[red]Error:[/red] Extension name is required for catalog installs"
                    )
                    raise typer.Exit(1)
