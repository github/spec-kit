...
# Fixed dispatcher stub written to a project's canonical
# scripts/bash/<name>.sh when that script name's priority stack requires
# composition (a "wrap" top layer). It carries no stack-specific data —
# only the script's own name, which is stable across preset installs,
# removals, priority changes and enablement toggles — so it never needs
# rewriting once installed; it resolves the live chain fresh on every
# invocation via `specify preset script-chain` (#4551).
_SCRIPT_CONTINUATION_DISPATCHER_TEMPLATE = """#!/usr/bin/env bash
# Generated by specify: continuation dispatcher for the "{script_name}"
# script. Do not edit directly — customize via presets/overrides instead
# (see `specify preset add`, `.specify/templates/overrides/scripts/`).
set -e
set -o pipefail
...
# Strip carriage returns immediately: on Windows, `specify`'s stdout
# carries CRLF line endings, and bash's $(...) only strips trailing
...
if [[ -n "$REMAINING" ]]; then
    ...
    export CORE_SCRIPT="$SCRIPT_DIR/continuation-runner.sh"
fi
...
class PresetManager:
    ...
    def _reconcile_script_chain(self, script_name: str) -> None:
        ...
        Unlike commands (see ``_reconcile_composed_commands``), a script's
        canonical file is executed rather than merely read by an agent, so
        composition doesn't need to be re-spliced every time the priority
        stack changes: a small continuation dispatcher resolves the live
        chain via ``specify preset script-chain`` at *invocation* time
        (#4551), so later `specify preset enable/disable/set-priority`
        calls take effect without touching this file again. Only install
        and remove — which change *which* layers exist at all, not merely
        their order — ever need to call this.

        Writes ``<project_root>/scripts/bash/<script_name>.sh``:
        - A single-layer chain (no composition) is a verbatim copy of that
          layer's file, so overrides/extensions take effect even without
          a ``"wrap"`` script in the stack.
        - A multi-layer chain is the fixed dispatcher stub below. It only
          resolves the chain and execs the top layer, so growing or
          shrinking the chain later (further installs/removes, or a
          priority/enablement change) never requires rewriting it.

        Does nothing when the script name has no resolvable layers at all
        (leaves whatever canonical file, if any, already exists alone).
        """
        resolver = PresetResolver(self.project_root)
        ...
        canonical = (
            self.project_root / ".specify" / "scripts" / "bash" / f"{script_name}.sh"
        )
        canonical.parent.mkdir(parents=True, exist_ok=True)

        if len(chain) == 1:
            content = chain[0].read_text(encoding="utf-8")
...
class PresetResolver:
    ...
    def resolve_script_chain(self, script_name: str) -> List[Path]:
        ...
        """
        layers = self.collect_all_layers(script_name, "script")
        if not layers:
            return []
        base_idx = next(
        ...
        if base_idx is None:
            return []
        return [layer["path"] for layer in layers[: base_idx + 1]]
