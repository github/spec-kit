...
# ///
"""
Diagramly CLI - Setup tool for Diagramly projects

Usage:
    uvx diagramly-cli.py init <project-name>
    uvx diagramly-cli.py init .
    uvx diagramly-cli.py init --here

Or install globally:
    uv tool install --from diagramly-cli.py diagramly-cli
    diagramly init <project-name>
    diagramly init .
    diagramly init --here
"""
...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'diagramly --help' for usage information[/dim]"))
        console.print()
...
def check_tool(tool: str, tracker: StepTracker = None) -> bool:
    ...
    # See: https://github.com/github/diagramly-kit/issues/123
...
def init_git_repo(project_path: Path, quiet: bool = False) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", "Initial commit from Diagramly template"], check=True, capture_output=True, text=True)
        if not quiet:
...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    repo_owner = "danshuitaihejie" #"github"
    repo_name = "diagramly-kit" #"spec-kit"
    if client is None:
        client = httpx.Client(verify=ssl_context)
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .diagramly/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".diagramly" / "scripts"
...
def init(
    ...
    Examples:
        ...
        diagramly init my-project --ai copilot --no-git
        diagramly init --ignore-agent-tools my-project
        diagramly init . --ai claude         # Initialize in current directory
        diagramly init .                     # Initialize in current directory (interactive AI selection)
        diagramly init --here --ai claude    # Alternative syntax for current directory
        diagramly init --here --ai codex
        diagramly init --here --ai codebuddy
        diagramly init --here
        diagramly init --here --force  # Skip confirmation when current directory not empty
    """
    ...
    setup_lines = [
        "[cyan]Diagramly Project Setup[/cyan]",
        "",
    ...
    tracker = StepTracker("Initialize Diagramly Project")
...
def check():
    ...
    console.print("\n[bold green]Diagramly CLI is ready to use![/bold green]")
