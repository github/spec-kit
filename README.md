...
| 预设 | 默认状态 | 你会用它做什么 |
| --- | --- | --- |
| `workflow-preset` | `specify init` 默认安装 | 增强 `/speckit.plan` 和 `/speckit.tasks`，并把 `/speckit.implement` 改成 agent-native handoff 编排流程，适合复杂功能和多 worker 执行。 |
...
`workflow-preset` 会保留 Spec Kit 的核心路径，但让复杂实现更容易拆分：

- `/speckit.plan` 可以补充对象设计、服务时序和测试策略等设计产物。
- `/speckit.tasks` 会读取这些设计产物生成更稳的任务。
- `/speckit.implement` 会生成 handoff manifest、worker handoff、context digest 和 receipt，让多 agent 或多阶段实现更可审查。
...
- 对中大型功能，保留默认 `workflow-preset`，让实现阶段通过 handoff 和 receipt 留下证据。
